...
Oliver's personal Codex plugins, skills, and marketplace — all in one repo.

## Structure

```
plugins/<name>/                          — Codex plugins (each has .Codex-plugin/plugin.json)
plugins/ames-standalone-skills/skills/   — Standalone skills (edit here directly, no build step)
.Codex-plugin/marketplace.json          — Marketplace manifest (auto-generated by ./sync)
...
| `ames-standalone-skills` | 26 standalone skills covering writing, dev, iOS, finance, automation, and AI | skills |
...
- `version` is required — bump it whenever content changes so Codex detects an update
...
Registered in Codex as `ames-Codex` pointing to `oliverames/ames-Codex`.

After any change: run `./sync`, commit, push. Codex will detect the update on next marketplace refresh.
