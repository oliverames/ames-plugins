...
Also expanded the skill with patterns surfaced during the same audit. (1) `categorization-workflow.md`'s "Splitting transactions" section now documents the delete + recreate workaround — `update_transaction` and `update_transactions` don't accept a `subtransactions` field, so converting a single transaction to a split requires `delete_transaction(id)` followed by `create_transaction({...subtransactions: [...]})` with a fresh `importId` to anchor deduplication; verified live during the audit on a $100 passport check split 50/50 between Henry and Emmett. (2) New "Common audit patterns" section in `SKILL.md` covering: pending returns workflow (yellow flag + `Pending return —` memo + optional move to Reimbursements), manual entry for non-imported transactions (Venmo, checks, cash — `create_transaction` with `cleared: "uncleared"` and no `importId`, lets bank reuse its own importId on next sync), transfer-pair cleared-state diagnostic (decision matrix for outflow×inflow cleared states with diagnostic procedure to confirm an "uncleared" transfer is legitimate vs. phantom), and payee-category drift from auto-categorization (Apple bundling via per-amount decoder table, City-of-X disambiguation via `import_payee_name_original` reading the raw bank descriptor).

**Decisions made**:
- **No plugin version bump for `ames-standalone-skills` (held at 3.14.0).** The skill change is a guidance correction, not a behavior/surface change. A patch bump would force-invalidate the marketplace cache (keyed on plugin version) across active Claude Code sessions for downstream consumers without adding any new capability. Leaving the version and letting the next substantive change carry the bump.
- **Connector got a patch bump (1.8.0 → 1.8.1).** `ynab-mcp-server` has its own versioning + npm publish cycle; tool descriptions are part of its public surface. Bumped both `package.json` and the in-file `McpServer` version (index.js:160) for consistency. Companion WORKLOG entry in that repo carries the full v1.8.1 detail.
- **Skipped `npm test` verification on ynab-mcp-server** due to a pre-existing Node v26 + MCP SDK ajv compatibility issue: `ajv/dist/ajv.js` doesn't resolve via Node 26's legacy main resolver inside `@modelcontextprotocol/sdk/dist/esm/validation/ajv-provider.js`. Test failure is environmental, not introduced by this change. The connector was used 30+ times in this same session against the live YNAB API to confirm runtime behavior, which substitutes for the test pass at the description-only-change level.
- **Did not edit the cached plugin copies** in `.claude/plugins/cache/...` or `.codex/plugins/cache/...`. Those refresh on next plugin reload from the marketplace.

**Verification**:
- Functional verification of match_broken approval via the very YNAB MCP tool whose description was being corrected: Apple Watch $45.33 (id `d9e7c3c2-1067-4b4c-a784-9f6c7a58a8c1_2026-04-30`) approved cleanly through `update_transaction` despite carrying the `match_broken` flag — API returned `approved: true` in the response object.
- Both commit messages cross-reference each other; both repos pushed and tracking `origin/main` clean.
- Marketplace clone at `~/.claude/plugins/marketplaces/ames-plugins/` re-synced via stash + rebase; matches `origin/main`.

**Left off at**: Both commits on `origin/main`. `ames-plugins` clean (`7801e8e` on top of `d07ede9`). `ynab-mcp-server` clean (`2de53d2` on top of `f3939f0`). Marketplace clone clean. Memory updates in this session's auto-memory (under `~/.claude/projects/-Users-oliverames-My-Drive-Financial-Planning/memory/`): `apple-charge-decoder.md` (new — 7 recurring Apple amounts mapped to YNAB categories), `user_income_sources.md` (updated with severance winding-down + Kaitlin's MRPS), `ynab-skill-clarifications.md` (new — caches the match_broken correction for fast recall).

**Open questions**:
- **NEW**: Should `update_transaction`/`update_transactions` tool descriptions explicitly note that `subtransactions` is NOT in their schemas (the convert-single-to-split workflow requires `delete_transaction` + `create_transaction` with subtransactions)? The skill's `categorization-workflow.md` now documents this; whether redundancy at the MCP tool description level is worth the extra description bytes is debatable. Worth one more pass if a fresh session repeats the mistake.
- **NEW**: Node v26 + MCP SDK ajv compatibility in `ynab-mcp-server`'s test environment. The legacy main resolver doesn't find `ajv/dist/ajv.js`; may need a fresh `npm install`, an explicit `"exports"` resolution path in the SDK, or a Node 22/24 fallback in `engines`.
- **Carried (still open)**: AP-style em-dash phrasing review (verbatim AP Stylebook check pending). 5 Apple Notes reference `ames-claude`; 4 plist preferences hold the literal string `ames-claude`; Cowork session state has stale refs. Same status as prior sessions — self-healing or low-priority.

---

## 2026-05-15 (late night) — AP Style skill + Merriam-Webster Collegiate MCP server (commits bc3959e + 595f0dc)

**What changed**: Two shipped deliverables enabling editorial workflow inside Claude Code. (1) New `ap-style` skill in `ames-standalone-skills`, paraphrasing established AP Stylebook rules with original examples — numbers, dates, titles, attribution, quotation, capitalization, 2014–2020 AP updates, and a pre-publish checklist. Defers to `bcbs-brand` for Blue Cross VT house style where it diverges (e.g., `healthcare` vs `health care`). Adds `skills/ap-style/data/ap/` for Oliver's verbatim Stylebook excerpts; folder is gitignored except for the README so proprietary AP content never ships in the public marketplace. Bumped `ames-standalone-skills` 3.13.3 → 3.14.0. (2) New `merriam-webster` MCP server in `ames-general-mcps` wrapping the Merriam-Webster Collegiate Dictionary + Collegiate Thesaurus APIs. Five tools: `define`, `synonyms`, `antonyms`, `spell`, `lookup` (combined). Single Python file using PEP 723 inline deps + FastMCP + async httpx, launched by `uv run --script` — no install step. Bumped `ames-general-mcps` 3.1.1 → 3.2.0. Both keys stored in 1Password Development vault (*Merriam-Webster Collegiate Dictionary API Key*, *Merriam-Webster Collegiate Thesaurus API Key*), with plaintext copies in `~/.claude/settings.json` env block (for Claude Code's MCP launcher) and `op://` refs in `~/.claude/.env` (for backup/restore parity). Cross-referenced from `ap-style` SKILL.md's "When you're not sure" section so the skill knows to reach for MW as its first stop on word-level questions. Also pulled top-level README + CLAUDE.md counts forward (skill total 51 → 59, MCP total 14 → 15) and fixed pre-existing version drift in the README plugin table (1.0.1 → 1.0.2 on three Codex-converted plugins, 3.10.0 → 3.14.0 on standalone, 3.1.0 → 3.2.0 on general-mcps).

**Decisions made**:
- **Authorship-vs-use-case placement test on the MW MCP.** Per memory `feedback_split_by_use_case`, plugin placement follows when-Oliver-reaches-for-it, not who-authored-it. MW is general-purpose word lookup (relevant outside editorial too), so it stays in `ames-general-mcps`. `ames-connectors` (despite "first-party" framing) holds business-tool integrations, which MW isn't. Oliver confirmed after a three-option AskUserQuestion.
- **Skill content is paraphrased AP, not verbatim.** AP retains all rights to the Stylebook; even though Oliver owns a licensed copy, redistributing AP entries in a public marketplace skill would breach AP's terms. SKILL.md ships paraphrased rules with original examples; verbatim excerpts go in the gitignored `data/ap/` folder.
- **PEP 723 inline deps over a separate venv.** Cleaner than tannmaycoding/Dictionary-MCP's venv pattern; no install step, deps materialize on `uv run --script`. Smoke-tested: 36 deps installed in 73ms on first run.
- **Implementation is a rewrite, not a fork.** Different API surface (dictionary + thesaurus, not dictionary-only), async vs blocking, structured JSON returns vs printed strings, and an added `lookup` combined tool. No code or LICENSE inherited from upstream.

**Verification**:
- Both commits pushed; `git status` clean on `main` matching `origin/main`.
- All four marketplace JSON files (Claude + Codex × top-level + per-plugin) parse cleanly via `python3 -m json.tool`.
- MCP protocol smoke test against the new server: stdio handshake confirms server initializes as `"merriam-webster"`, all 5 tools register via `tools/list`.
- Direct API smoke test against both keys with `httpx`: `define("test")` returns dictionary entry with `shortdef`; `synonyms("test")` returns 2 synonym groups; misspell test (`"scrootiny"`) returns expected suggestions array.
- `${CLAUDE_PLUGIN_ROOT}` substitution in `.mcp.json` confirmed against Anthropic's official plugin reference docs (their canonical example uses the same pattern).
- `./sync` propagated both version bumps to Codex manifests; regenerated `agents/openai.yaml` for `ap-style` (manual title-case fix: "Ap Style" → "AP Style").

**Open questions**:
- **NEW**: MW MCP needs a fresh Claude Code session to enter the plugin cache. Current session predates v3.2.0, so the new server isn't loaded yet. Verification command after restart: ask for a `define` call on a known word.
- **NEW (low priority)**: The `ap-style` SKILL.md phrases em dashes as "AP uses em dashes sparingly." AP allows them freely for abrupt thought change or to set off a series; "sparingly" is editorial advice, not a Stylebook rule. Worth a glance against Oliver's licensed copy when convenient.
- **Carried (still open)**: 5 Apple Notes reference `ames-claude`; 4 plist preferences hold the literal string `ames-claude`; Cowork session state has stale refs. Same status as prior session — self-healing or low-priority.

**Left off at**: Both shipping artifacts are durable on `origin/main`. Top-level docs current. Settings.json and .env were modified outside this repo; backup via `Scripts/backup-claude` is the wrap-up step. Restart of Claude Code is the only remaining action to land the MW MCP into the live tool list.
...
- **Untracked residual in `ames-plugins`** (intentional): 7 `.backups/*.pre-1.8.0` (rollback path), 2 new reference files `data/letterhead/markdown-shape.md` and `data/source-documents-on-disk.md` (created in v1.7.1 progressive-disclosure pass), 1 new script `scripts/affordability-stat-check.py` (created in v1.8.0).
...
- **Untracked residual in `ames-plugins`**: `.backups/*.pre-1.7.0` (intentional); `skills/verify-live/` (created earlier today by a separate session per memory `project_verify_live_skill.md`, not mine to ship).
...
**Validated by**: BCBS VT Response Management Strategy doc on 2026-04-27 — first build via `build-letterhead.sh` (no flags) produced the wrong presentation-style look; rebuild via cloned strategy-memo exemplar produced the right tight memo style. The skill change makes the second build the default for similar future asks.
...
- Sanitization of `app.xml` zeros the page/word/character counters (Word recalculates on first save) — should `core.xml`'s `cp:revision` field also be reset to 1 in the exemplar? Currently inherits the source's value of 14, which is misleading for a "fresh" template.
...
**Left off at**:
- **NEW: Codex cache at `~/.codex/plugins/cache/ames-plugins/ames-standalone-skills/3.5.4/` still shows skill v1.1.0.** The iCloud source now has v1.2.0 and the Claude cache at 3.5.3 has v1.2.0. Codex cache will update on next plugin refresh; did not touch directly because cache writes get reverted on sync.
...
- **Codex marketplaces should be Git, not local paths** — user explicitly corrected this mid-session when `codex plugin marketplace upgrade ames-plugins` failed. Saved as `feedback_codex_marketplace_git.md` memory. Also saved the `config.toml.backup-YYYYMMDD-HHMMSS-<purpose>` pattern as `feedback_codex_config_backup.md`.

**Left off at**:
- **NEW: Two `codex-doctor` warnings remain, informational only** — "Codex cache for ames-dev-mcps is not at v1.0.1" and "...ames-general-mcps is not at v3.0.1". They fire because those plugins aren't enabled in `~/.codex/config.toml`, so their content isn't cached. To start using them on Codex, add `[plugins."ames-dev-mcps@ames-plugins"]` / `[plugins."ames-general-mcps@ames-plugins"]` with `enabled = true`.
- **NEW: `ames-connectors` still has no LICENSE file.** `ames-plugins` now has one; the sibling repo should match for the same legal-clarity reason (plugin.jsons declare MIT but no LICENSE backs the claim).
...
- **Resolved this session**: `bump-and-sync` retirement-or-not question (now significantly hardened; less urgent to retire), orphan dot-dirs under `skills/` (moved out), stale `ames-preferred-mcps@ames-plugins` in Codex config (removed), local→Git marketplace migration for `ames-plugins`.
...
- **Flagged settings.json drift in the README rather than silently fixing it.** The user's `~/.claude/settings.json` still references `ames-preferred-mcps@ames-plugins` — I updated the documented snapshot to say "3 of 6" and called out that the stale key needs replacing on the next session, rather than editing settings.json mid-wrap-up (settings.json wasn't my file to modify and the user has their own drift-check ritual).
...
**Left off at**:
- **NEW: `~/.claude/settings.json` `enabledPlugins` still lists the old `ames-preferred-mcps@ames-plugins` key.** Next session should replace with `ames-dev-mcps@ames-plugins` and `ames-general-mcps@ames-plugins`. The stale key will be inert (plugin no longer exists in the marketplace) but the user's config record in README reflects the drift.
...
- **NEW: Verify installed plugin caches refresh after the rename.** After marketplace auto-update, `~/.claude/plugins/cache/ames-plugins/ames-preferred-mcps/` will become orphaned; the `ames-dev-mcps/` and `ames-general-mcps/` caches will populate on next Claude Code launch.
...
- **Added sync script to ames-connectors rather than leaving marketplace.json hand-edited.** Mirrors the ames-plugins pattern. Trivial to maintain once, pays off on every future bump. The ames-connectors `sync` is identical to ames-plugins's except for the header string — a deliberate copy, not shared code, since the two repos should stay operationally independent.
- **Patch bumps, not minor.** Per ames-plugins version policy (patch = content tweaks), metadata enrichment is a patch. Minor would have been reserved for new skills/commands/MCPs.
- **Minor bump for marketplace metadata.version.** 3.4.0→3.5.0 (ames-plugins) and 1.1.0→1.2.0 (ames-connectors). Signals materially-changed metadata to indexers, and marks the boundary between "flat" and "enriched" marketplace content. Plugin-level patches roll up to a marketplace minor.
...
- **Didn't touch Apple Notes in the Tech folder.** Both `🧰 ames-plugins marketplace` and `🛒 Marketplaces` describe the "what" (5 plugins, dual-host layout); today's changes were about the "how" (metadata propagation mechanics). That belongs in CLAUDE.md + README, not the high-level reference notes.
...
- **NEW: Consider extracting the two sync scripts into a shared helper.** ames-plugins/sync and ames-connectors/sync are near-identical. If a third marketplace appears, promote to `~/Developer/Scripts/sync-marketplace` with a thin per-repo wrapper.
...
- **Resolved this session**: dangling `ames-plugins-only@ames-plugins` key in `~/.claude/settings.json` — no longer present (confirmed via grep), whether Codex cleaned it or a prior session did.

**Open questions**:
- Should `ames-connectors` get a local checkout at `~/Developer/Projects/ames-connectors` for parity with how `ames-plugins` lives on disk, or is Codex-only development the intended model? Currently only the Claude Code marketplace cache clone exists locally; the canonical is GitHub.
...
## 2026-04-17 (evening) — Promote Codex-derived skills to top-level plugins, retire ames-plugins-only

**What changed**: Refactored the marketplace layout to fix a nested-skill auto-discovery concern flagged in the afternoon session's review notes. Moved `build-ios-apps-codex` (6 skills) and `build-macos-apps-codex` (11 skills + 3 commands) from nested bundles at `plugins/ames-plugins-only/skills/<bundle>/skills/<sub>/` up to top-level plugins at `plugins/<bundle>/skills/<sub>/`. Used `git mv` to preserve per-file history (74 renames tracked as R entries). Deleted the redundant router `SKILL.md` wrappers (~131 lines combined) that sat at each bundle level and only pointed at the sub-skills. Retired the `ames-plugins-only` umbrella plugin entirely (`git rm -rf`) — its sole purpose was housing those two bundles. Bumped `.claude-plugin/marketplace.json` `metadata.version` 3.2.0 → 3.4.0. Ran `/simplify` with three parallel review agents (reuse, quality, efficiency), which caught 9+ stale references to `ames-plugins-only` in `README.md` I'd missed: install examples, plugin table, dedicated section, architecture table, skill-count summary, gotchas, version, enabled-plugins snapshot, upstream credit. Also caught a stale example in `sync`'s pass-3 comment, a misleading `# Sync skills (skip openai.yaml agent files)` comment in both `update.sh` scripts (loop doesn't actually filter yaml; skips by omission), and drift-risk in the new plugin.json descriptions that enumerated every skill name. Fixed all. Shortened plugin.json descriptions to match `ames-standalone-skills` style. Trimmed the CLAUDE.md plugin-table rows for the two new plugins to match sibling-row terseness. Updated Apple Notes `💻 Tech/🧰 ames-plugins marketplace` from "6 plugins, 31 skills" to "7 plugins, 46 skills" with the retirement note. Committed as `2b51... refactor(marketplace): promote Codex-derived skills to top-level plugins` plus a follow-up `2ad0d1f chore(codex-sync): drop misleading "skip openai.yaml" comment` for two `update.sh` edits that slipped past staging because `git mv` captured the rename before the Edit touched content. Pushed both.
...
- **Retire `ames-plugins-only` entirely rather than repurpose as an empty shell.** Presented three options (delete, keep as placeholder, keep as migration breadcrumb). User chose delete. The CLAUDE.md convention elsewhere is "no empty plugins in the marketplace"; git history preserves the record if it ever needs resurrection.
...
- **NEW: `~/.claude/settings.json` has a dangling `"ames-plugins-only@ames-plugins": true` key** pointing at a plugin that no longer exists. Either remove it (retiring that row) or replace with `"build-ios-apps-codex@ames-plugins": true` and `"build-macos-apps-codex@ames-plugins": true` to enable the two new plugins. Out of scope for this session; a future `wrap-up` config-drift pass should reconcile.
...
- **Exclude `ames-plugins-only` from the Codex manifest.** A plugin whose skills were converted from Codex plugins can't honestly claim to be installable in Codex. Omitting preserves the invariant that every plugin in the Codex marketplace actually works there.
...
- NEW: Consider adding a `LICENSE` file to `ames-plugins`. The `/readme-style` skill's badge row expects a license badge, which I omitted because no license file exists. MIT would match the ecosystem convention but requires explicit user approval.
- NEW: Consider an icon/logo asset for the ames-plugins README centered header. Currently the header is text-only; a small SVG would strengthen `/readme-style` compliance.
...
**Open questions**:
- When `ames-plugins-only`'s upstream OpenAI skills change, what's the right cadence for reconversion? Currently manual; no automation exists. Worth considering a `sync-upstream-codex-skills` script.
...
**What changed**: Applied consistent terminal styling to the three ames-plugins scripts (`sync`, `bump-and-sync`, `plugins/ames-preferred-mcps/sync-sources`) and added `style.sh` (shared library). Python scripts got inline ANSI color helpers matching the same palette. Part of a broader session that also overhauled all 10 scripts in `Developer/Scripts` (not a git repo — iCloud only).

**Decisions made**: `sync` and `sync-sources` are Python so they can't source `style.sh` — used the same inline color helper pattern established in `redact-keys`. `style.sh` added to the ames-plugins root so `bump-and-sync` (bash) can source it. Style is cosmetic only; all logic was preserved.
...
- Still open (carried): disable ames-original-connectors@ames-plugins / enable ames-ynab@ames-plugins in plugin UI after next marketplace refresh
- Still open (carried): ImageRelay 1Password fallback verification
- Still open (carried): UniFi — create 1Password item to bring online
- Still open (carried): verify iMCP/SimGenie/Sosumi appear in `claude mcp list` under `plugin:ames-preferred-mcps:*`
...
**Left off at**: Still need two manual Apple Notes edits in "💻 Tech" folder: (1) "💻 My Tech Stack" -- change `ames-skills repo — Plugins and skills marketplace (94 skills across 10 plugins)` → `ames-plugins repo — Plugins and skills marketplace (30+ skills across 11 plugins)`; (2) "🔧 My Claude Code Setup" -- change `sync-skills — install plugins/skills from ames-skills repo` → `ames-plugins repo`. Still need to add `BEAR_API_TOKEN` to `~/.claude/settings.json` env section. Still need to run `sync-skills` to install the new `@ames-plugins` structure locally and trigger the legacy `@ames-skills` cleanup. Still open from previous sessions: Axiom skills interaction verification, swiftui-pro iOS 26 content update.
