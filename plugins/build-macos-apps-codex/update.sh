#!/bin/bash
# update.sh — Sync build-macos-apps-codex from local Codex plugin cache

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SKILL_DIR="$SCRIPT_DIR"
CODEX_CACHE="$HOME/.codex/plugins/cache/openai-curated/build-macos-apps"
PLUGIN_NAME="build-macos-apps"
...
banner "build-macos-apps-codex synced from Codex v$CODEX_VERSION"
