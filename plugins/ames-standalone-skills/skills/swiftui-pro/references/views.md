...
- Apply `.glassEffect(in: .rect(cornerRadius: 16))` to container views for glass backgrounds. Apply padding *outside* the glass effect so clipping is correct.
...
- Use `.glassEffect(.regular.interactive())` for interactive glass that responds to touch. Disable with `isEnabled: false`.
- For morphing between glass shapes, keep content in the same container and use `@Namespace` + `matchedGeometryEffect` on the glass container.
- Choose controls semantically, then apply glass styling:
  - `.buttonStyle(.glass)` for standard glass buttons
  - `.buttonStyle(.glassProminent)` for emphasized glass buttons
  - Never reimplement glass button visuals manually
