...
| URL | `URL` |
| Get Contents of URL / Download URL | `Contents of URL` |
| Number | `Number` |
| Date | `Date` |
| List | `List` |
| Dictionary | `Dictionary` |
| Detect Dictionary | `Dictionary` |
| Get Dictionary Value (getvalueforkey) | `Dictionary Value` |
| Repeat Each (inside loop) | `Repeat Item` |
| Repeat Each END (after loop) | `Repeat Results` (list of all iteration outputs) |
| Repeat Count | `Repeat Index` |
| Choose from List | `Chosen Item` (single) or `Selected Item` (Tailscale picker) |
| Count | `Count` |
| Convert Image (image.convert) | `Converted Image` |
| Filter Notes (filter.notes) | `Note` |
| Run JavaScript on Web Page | `JavaScript Result` |
| Run Shell Script | `Shell Script Result` |
| Run Script over SSH | `Shell Script Result` |
| Base64 Encode/Decode | `Base64 Encoded` |
| Save File (documentpicker.save) | `Saved File` |
| Rename File (file.rename) | `Renamed File` |
