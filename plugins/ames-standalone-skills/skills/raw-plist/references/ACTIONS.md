...
> **All Notes actions require `AppIntentDescriptor`** — without it they appear as "Unknown Action".

| Identifier | Description | AppIntentIdentifier |
|------------|-------------|---------------------|
| `filter.notes` | Find/list notes (sorted) — output name is `"Note"` (singular) | `NoteEntity` |
| `appendnote` | Append content to a note | `AppendToNoteLinkAction` |
| `shownote` | Open a note | *(no AppIntentDescriptor needed)* |

**Correct `AppIntentDescriptor` values:**
```python
# filter.notes
...
**`filter.notes` full parameter structure:**
```python
 ...
 "WFWorkflowActionParameters": {
     ...
     "WFContentItemFilter": {
         "Value": {
             ...
             "WFActionParameterFilterTemplates": [],          # empty = all notes
             "WFContentItemSortProperty": "Title",
             "WFContentItemSortOrder": "Ascending"
         },
         "WFSerializationType": "WFContentPredicateTableTemplate"
     },
     "AppIntentDescriptor": NOTES_FILTER_INTENT  # REQUIRED
 }}
# Output name: "Note" (singular — NOT "Notes")
```

**`appendnote` full parameter structure:**
```python
{"WFWorkflowActionIdentifier": "is.workflow.actions.appendnote",
 "WFWorkflowActionParameters": {
     "WFInput": action_ref(U_content, "Contents of URL"),  # or image/file output
     "WFNote": var_ref("DestNote"),                        # variable holding the note
     "AppIntentDescriptor": NOTES_APPEND_INTENT            # REQUIRED
 }}
```

> **NEVER use `is.workflow.actions.findnotes`** — this identifier does not exist and will always
> produce "Unknown Action". The correct identifier is `is.workflow.actions.filter.notes`.
