...
REQUIRE_WRAPPED_INPUT_ACTIONS: set[str] = set()
    ...
    for idx, act in enumerate(actions):
        ...
        if ident == "is.workflow.actions.setvariable":
            ...
            if isinstance(name, str) and name.strip():
                ...
                if out_uuids:
                    source_uuid = out_uuids[0]
                    if uuid_to_ident.get(source_uuid) == "is.workflow.actions.format.date":
        ...
        if ident in REQUIRED_INPUT_ACTIONS:
            ...
            elif ident in REQUIRE_WRAPPED_INPUT_ACTIONS:
                if _input_action_output_uuids(wfinput) and not _conditional_input_is_wrapped(wfinput):
                    errors.append(
                        f"WFInput should use Type=Variable wrapper for editor visibility at index {idx}: {ident}"
                    )
            else:
